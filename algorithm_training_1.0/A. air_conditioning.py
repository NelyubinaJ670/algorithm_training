...
есть всего лишь два управляемых параметра: желаемая температура и режим работы.
...
Поскольку температура в комнате ниже, чем желаемая, кондиционер самостоятельно 
выключается и температура в комнате не поменяется.

"""

troom, tneed = map(int, input().split())
mode = input()

if mode == 'freeze':
    print(min(troom, tneed))
if mode == 'heat':
    print(max(troom, tneed))
if mode == 'fan':
    print(troom)
if mode == 'auto':
    print(tneed)
